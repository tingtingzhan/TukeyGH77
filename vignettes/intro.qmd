...
-   `true`, the true simulating parameter $g$
-   `demo`stration, of estimates $(\hat{B}, \hat{h})$, $(\hat{B}_{\text{L}}, \hat{h}_{\text{L}})$ and $(\hat{B}_{\text{U}}, \hat{h}_{\text{U}})$ based on all available choices of $\hat{g}$

@fig-gh54 shows a 'good' data set that

-   $\hat{g}_p$ does not have a clear pattern (@fig-gh54 `E`);
-   we have very small $\text{err}_h$ and $\text{err}_B$ with the naive `median` choice of $\hat{g}$ (@fig-gh54 `A`);
-   difference choice of $\hat{g}$, e.g., `B_optim`, `h_optim` or `true` (@fig-gh54 `B`, `C`, `D`), does not affect much the estimates $\hat{B}$ and $\hat{h}$.
...
-   $\hat{g}_p$ has a clear pattern (@fig-gh335 `E`);
-   estimated slope $\hat{h}<0$ for `median` and `B_optim` choices of $\hat{g}$ (@fig-gh335 `A`, `B`)
-   estimated slope $\hat{h}>0$ for `h_optim` choice of $\hat{g}$ (@fig-gh335 `C`).
...
-   $\hat{g}_p$ has a clear pattern (@fig-gh39 `E`);
-   estimated slope $\hat{h}<0$ for all choices of $\hat{g}$, including the true simulating parameter $g$ (@fig-gh39 `A`, `B`, `C`, `D`).
