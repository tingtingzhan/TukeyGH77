...
We explore some choices of $\hat{A}$ between the $45$th to $55$th sample percentile, with parameter `A_select` of function `letterValue_h()`,

-   `A_select = 'median'` (default) of random sample $T$, the ***naive*** choice
-   `A_select = 'B_optim'`, to minimize $\text{err}_B =(\hat{B}_{\text{U}}-\hat{B}_{\text{L}})^2$
-   `A_select = 'h_optim'`, to minimize $\text{err}_h =(\hat{h}_{\text{U}}-\hat{h}_{\text{L}})^2$
...
We first explore some choices of $\hat{A}$ between the $45$th to $55$th sample percentile, with parameter `A_select` of function `letterValue_g()`,

-   `A_select = 'median'` of random sample $T$, the ***naive*** choice
-   `A_select = 'optim'` (default), to minimize the standard deviation of $\hat{g}_p$
...
We then explore some choices of $\hat{g}$ between the $40$th to $60$th percentile of $\hat{g}_p$, with parameter `g_select` of function `letterValue_g()`,

-   `g_select = 'median'` of $\hat{g}_p$, the ***naive*** choice
...
From Tukey's $g$-&-$h$ transformation on LHS and UHS, as well as their difference and ratio (Upper: p487,eq(33). Lower: see p486,bottom),
...
Similar to Tukey's $g$-model, we first explore some choices of $\hat{A}$ between the $45$th to $55$th sample percentile, with parameter `A_select` of function `letterValue_gh()`,

-   `A_select = 'median'` of random sample $T$, the ***naive*** choice
-   `A_select = 'optim'` (default), to minimize the standard deviation of $\hat{g}_p$.


We then explore some choices of $\hat{g}$ between the $40$th to $60$th percentile of $\hat{g}_p$, with parameter `g_select` of function `letterValue_gh()`,

-   `g_select = 'median'` of $\hat{g}_p$, the ***naive*** choice
-   `g_select = 'B_optim'`, to minimize $\text{err}_B =(\hat{B}_{\text{U}}-\hat{B}_{\text{L}})^2$
-   `g_select = 'h_optim'` (default), to minimize $\text{err}_h =(\hat{h}_{\text{U}}-\hat{h}_{\text{L}})^2$
...
#| fig-width: 6
#| fig-align: left
#| fig-cap: 'Perfect data set: choices of A'
...
#| fig-width: 7
#| fig-align: left
#| fig-cap: 'Perfect data set: choices of g'
set.seed(54); rGH(n = 1e3L, g = -.3, h = .1) |> 
...
-   the `'optim'` choice of $\hat{A}$ not only reduces the standard deviations of $\hat{g}_p$, but also removes the clear pattern in $\hat{g}_p$, @fig-gh335-A;
-   the `'h_optim'` choice of $\hat{g}$ provides decent estimated $\hat{h}$, @fig-gh335.
...
#| fig-width: 6
#| fig-align: left
#| fig-cap: 'Salvageable data set: choices of A'
...
#| fig-width: 7
#| fig-align: left
#| fig-cap: 'Salvageable data set: choices of g'
set.seed(335); rGH(n = 1e3L, g = -.3, h = .1) |>
  letterValue_gh(g_select = 'demo')
```
...
-   the `'optim'` choice of $\hat{A}$ neither reduces much the standard deviations of $\hat{g}_p$, nor removes the clear pattern in $\hat{g}_p$, @fig-gh39-A;
...
#| fig-width: 6
#| fig-align: left
#| fig-cap: 'Hopeless data set: choices of A'
...
#| fig-width: 7
#| fig-align: left
#| fig-cap: 'Hopeless data set: choices of g'
set.seed(39); rGH(n = 1e3L, g = -.3, h = .1) |>
  letterValue_gh(g_select = 'demo')
```

Good new is that we do not see 'hopeless' data sets often in simulations. In this case, we recommend reporting $\hat{h}=0$.
