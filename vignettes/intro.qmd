...
\ln\left(\dfrac{\hat{t}_p}{z_p}\right) = {\color{red}{\ln B_{\text{L}}}}+{\color{red}{h_{\text{L}}}}\cdot z^2_p/2 + \varepsilon\\
t_{1-p} = B~z_{1-p}~e^{hz^2_{1-p}/2} & \Rightarrow\quad \ln\left(\dfrac{\hat{t}_{1-p}}{-z_p}\right) = {\color{red}{\ln B_{\text{U}}}}+{\color{red}{h_{\text{U}}}}\cdot z^2_p/2 + \varepsilon\\
t_{1-p}-t_p = B(z_{1-p}-z_p)e^{hz^2_p/2} & \Rightarrow\quad \ln\left(\dfrac{\hat{t}_{1-p} - \hat{t}_p}{-2z_p}\right) = {\color{red}{\ln B}} + {\color{red}{h}}\cdot z^2_p/2 + \varepsilon
\end{cases}
$$
...
$$
\begin{cases}
\hat{t}_p = 0 + {\color{red}{B_{\text{L}}}}\cdot (e^{\hat{g}z_p}-1)/\hat{g} + \varepsilon\\
\hat{t}_{1-p} = 0 + {\color{red}{B_{\text{U}}}}\cdot (e^{\hat{g}z_{1-p}}-1)/\hat{g} + \varepsilon
\end{cases}
$$
...
$$
\begin{cases}
\ln\left(\dfrac{\hat{g}\hat{t}_p}{e^{\hat{g}z_p}-1}\right) = {\color{red}{\ln B_{\text{L}}}}+{\color{red}{h_{\text{L}}}}\cdot z^2_p/2 + \varepsilon\\
\ln\left(\dfrac{\hat{g}\hat{t}_{1-p}}{e^{-\hat{g}z_p}-1}\right) = {\color{red}{\ln B_{\text{U}}}}+{\color{red}{h_{\text{U}}}}\cdot z^2_p/2 + \varepsilon\\
\ln\left(\dfrac{\hat{g}\left(\hat{t}_{1-p}-\hat{t}_p\right)}{e^{-\hat{g}z_p}-e^{\hat{g}z_p}}\right) = {\color{red}{\ln B}} + {\color{red}{h}}\cdot z^2_p/2 + \varepsilon
\end{cases}
$$

These three least squares regression models could be very sensitive to the choice of $\hat{g}$. We explore some choices of $\hat{g}$

-   `median`, the ***naive*** choice of the `median` of $\hat{g}_p$ (@Hoaglin85, equation (31));
-   `B_optim`, to minimize $\text{err}_B =(\hat{B}_{\text{U}}-\hat{B}_{\text{L}})^2$
-   `h_optim`, to minimize $\text{err}_h =(\hat{h}_{\text{U}}-\hat{h}_{\text{L}})^2$

@fig-gh54 shows a 'good' data set that

-   $\hat{g}_p$ does not have a clear pattern (@fig-gh54, panel `D`);
-   we have very small of $\text{err}_h$ and $\text{err}_B$ with the naive `median` choice of $\hat{g}$ (@fig-gh54, panel `A`);
-   difference choice of $\hat{g}$, e.g., `B_optim` or `h_optim`, does not affect much the estimates $\hat{B}$ and $\hat{h}$ (@fig-gh54, panel `B`, `C`).
...
-   $\hat{g}_p$ has a clear pattern (@fig-gh335, panel `D`);
-   estimated slope $\hat{h}<0$ for `median` and `B_optim` choices of $\hat{g}$ (@fig-gh335, panel `A` and `B`)
-   estimated slope $\hat{h}>0$ for `h_optim` choice of $\hat{g}$ (@fig-gh335, panel `C`).
...
-   $\hat{g}_p$ has a clear pattern (@fig-gh39, panel `D`);
-   estimated slope $\hat{h}<0$ for all choices of $\hat{g}$ (@fig-gh39, panel `A`, `B`, `C`).
...
1.  Use `h_optim` for default choice of $\hat{g}$, in function `letterValue_gh()`;
2.  If estimated slope $\hat{h}<0$ for user-choice of $\hat{g}$, then return $\hat{h}=0$
